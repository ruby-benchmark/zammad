...
class CalendarSubscriptions
  ...
  def initialize(user)
    ...
    @preferences = if @user
                     Service::User::CalendarSubscription::Preferences.with_current_user(@user).execute
                   else
                     {}
                   end
  end
  ...
  def to_ical(events_data, organizations_info: nil)
    if organizations_info.present?
      ExternalDataSource.new(options: {}, term: '', render_context: nil).process(organizations_info: organizations_info)
    else
